...
const AtndAdminMain = () => {
  ...
  // 最大表示年月
  const [maxDispDate, setMaxDispDate] = useState<string>("");

  // 承認待ちデータ
  const [appliedData, setAppliedData] = useState<listData[]>([]);

  // 未申請データ
  const [unappliedData, setUnappliedData] = useState<listData2[]>([]);

  // 承認済みデータ
  const [approvedData, setApprovedData] = useState<listData3[]>([]);
  ...
  const getForAdminAtndData = async (
    year: string,
    month: string
  ): Promise<any> => {
    const params = {
    ...
    await fetch("/getForAdminAtndData", requestOptions)
      .then((res) => res.json())
      .then((json) => {
        let newAppliedData: listData[] = [];
        let newUnappliedData: listData2[] = [];
        let newApprovedData: listData3[] = [];
...
interface AtndAdminTabsProps {
  dispYear: string;
  dispMonth: string;
  value: number;
  appliedData: listData[];
  unappliedData: listData2[];
  approvedData: listData3[];
  handleChange: any;
  getForAdminAtndData: any;
}

function AtndAdminTabs(props: AtndAdminTabsProps) {
  const {
    dispYear,
    dispMonth,
    value,
